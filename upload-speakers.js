...
const serviceAccountKey = JSON.parse(fs.readFileSync(path.resolve('./serviceAccountKey.json'), 'utf8'));
...
async function seedSpeakers() {
  try {
    admin.initializeApp({
      ...
      databaseURL: 'https://eep-app-ea0e0.firebaseio.com'
    });
    ...
    const existingSpeakers = await speakersCollection.get();
    let deletedCount = 0;
    ...
    // Upload new speakers in batches
    let uploadedCount = 0;
    for (const speaker of speakers) {
      await speakersCollection.doc(speaker.id).set(speaker);
      uploadedCount++;
    }

    console.log(`✅ Successfully uploaded ${uploadedCount} speakers with LinkedIn links to Firestore!`);
